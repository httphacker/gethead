...
if response.info().getheader('x-frame-options'):
    if response.info().getheader('x-frame-options').lower() == 'deny' or 'sameorigin':
        ...
        printout('Vulnerability ', RED)
        printout('- Server does not enforce Cross-Frame Scripting Protection.\nThe X-Frame-Options Header setting is inadequate.\nClient may be vulnerable to Click-Jacking Attacks.\n\n', WHITE)
else:
    printout('Vulnerability ', RED)
    printout('- Server does not enforce Cross-Frame Scripting Protection.\nThe X-Frame-Options Header setting is missing.\nClient may be vulnerable to Click-Jacking Attacks.\n\n', WHITE)

# check x-content-type-options:
if response.info().getheader('x-content-type-options'):
    if response.info().getheader('x-content-type-options').lower() == 'nosniff':
        printout('(X-Content-Type-Options) MIME-Sniffing Protection is enforced.\n\n', GREEN)
    else:
        printout('Vulnerability ', RED)
        printout('- Server does not enforce MIME-Sniffing Protection.\nThe X-Content-Type-Options Header setting is inadequate.\nClient may be vulnerable to MIME-Sniffing Attacks.\n\n', WHITE)
else:
    printout('Vulnerability ', RED)
    printout('- Server does not enforce MIME-Sniffing Protection.\nThe X-Content-Type-Options Header setting is missing.\nClient may be vulnerable to MIME-Sniffing Attacks.\n\n', WHITE)
